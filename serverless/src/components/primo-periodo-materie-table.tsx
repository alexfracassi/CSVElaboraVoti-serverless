...
export function PrimoPeriodoMaterieTable({ materie }: PrimoPeriodoMaterieTableProps) {
  ...
  const getInsuffColor = (percentuale: number): string => {
    if (percentuale < 10) return 'bg-green-500';
    if (percentuale < 20) return 'bg-lime-500';
    if (percentuale < 30) return 'bg-yellow-500';
    if (percentuale < 40) return 'bg-orange-500';
    return 'bg-red-500';
  };
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Progress 
                          ...
                          className="h-2 w-24"
                          style={{
                            ['--progress-background' as string]: getInsuffColor(materia.percentualeInsuff)
                          }}
                        />
